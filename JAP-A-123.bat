...
SET MAINCLASSSRC=src/assign/Game.java
SET MAINCLASSBIN=assign.Game
SET IMAGES=images
...
copy "%SRCDIR%/%RESOURCES%" "%BINDIR%/%RESOURCES%" /Y
:: mkdir "%BINDIR%/%IMAGES%"
:: xcopy "%SRCDIR%/%IMAGES%" "%BINDIR%/%IMAGES%" /Y
mkdir "%BINDIR%"
mkdir "%BINDIR%\%PACKAGE%"
copy "%SRCDIR%\%PACKAGE%\*.png" "%BINDIR%\%PACKAGE%"
copy "%SRCDIR%\%PACKAGE%\*.png" "%BINDIR%"
copy "%SRCDIR%\%PACKAGE%\*.gif" "%BINDIR%\%PACKAGE%"

ECHO "1. Compiling ......................"
::javac -Xlint -cp ".;src;/javafx/lib/*;/SOFT/COPY/db/derby/lib/*" src/Lab.java -d bin 2> labs-javac.err
...
start java -cp "../%LIBDIR%;%BINDIR%;%SRCDIR%/resources" -Duser.language=en -Duser.country=US -jar %JARNAME%
cd ..
